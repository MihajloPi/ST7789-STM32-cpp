...
inline void ST7789::dcSet()    const { HAL_GPIO_WritePin(_dcPort,  _dcPin,  GPIO_PIN_SET);   }
inline void ST7789::dcClr()    const { HAL_GPIO_WritePin(_dcPort,  _dcPin,  GPIO_PIN_RESET); }
inline void ST7789::rstSet()   const { HAL_GPIO_WritePin(_rstPort, _rstPin, GPIO_PIN_SET);   }
inline void ST7789::rstClr()   const { HAL_GPIO_WritePin(_rstPort, _rstPin, GPIO_PIN_RESET); }
...
void ST7789::drawLine(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t color)
{
    ...
    bool steep = absVal(int16_t(y1-y0)) > absVal(int16_t(x1-x0));
    if (steep) { swap(x0, y0); swap(x1, y1); }
    if (x0 > x1) { swap(x0, x1); swap(y0, y1); }

    int16_t dx    = int16_t(x1 - x0);
    int16_t dy    = absVal(int16_t(y1 - y0));
    int16_t err   = dx / 2;
...
void ST7789::drawFilledTriangle(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2,
                                 uint16_t x3, uint16_t y3, uint16_t color)
{
    int16_t dx = absVal(int16_t(x2-x1)), dy = absVal(int16_t(y2-y1));
